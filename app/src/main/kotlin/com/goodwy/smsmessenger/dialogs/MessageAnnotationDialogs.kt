...
object MessageAnnotationDialogs {
 fun editLabels(activity: BaseSimpleActivity, message: Message, onSaved: () -> Unit) {
  ...
  ensureBackgroundThread{val labels=MessageAnnotationStore.getMessageLabels(activity,message.id);activity.runOnUiThread{input.setText(labels.joinToString(", "){ label -> label.name });AlertDialog.Builder(activity).setTitle(R.string.annotation_add_label).setView(container).setNegativeButton(android.R.string.cancel,null).setPositiveButton(android.R.string.ok){_,_->val names=input.text.toString().split(',', '\n','،');ensureBackgroundThread{MessageAnnotationStore.setMessageLabels(activity,message.id,names);activity.runOnUiThread(onSaved)}}.show()}}
 }
